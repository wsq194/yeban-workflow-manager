...
export const API = {
    // workflows - 自动保存（无弹框）
    list:          ()                              => post("/list"),
    save: (workflow_name, workflow_data, group_id) =>
        post("/save", { workflow_name, workflow_data, group_id }),
    
    // 手动保存（无弹框）- 直接使用标签栏名称
    saveQuick: (workflow_name, workflow_data, group_id) =>
        post("/save", { workflow_name, workflow_data, group_id }),
    
    // 另存为（有弹框）
    saveAs: (workflow_name, workflow_data, group_id) =>
        post("/save", { workflow_name, workflow_data, group_id }),
