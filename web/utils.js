 ...
 * 更新标签栏的工作流名称
 */
export function setActiveTabName(name) {
    try {
        if (window.app.extensionManager?.workflow?.activeWorkflow) {
            window.app.extensionManager.workflow.activeWorkflow.filename = name;
        }
    } catch (e) {
        console.warn("[yeban-WM] 更新标签栏失败:", e);
    }
}
 ...
 * 生成默认工作流名称
 */
export function generateDefaultWorkflowName() {
    const now = new Date();
