...
git clone https://github.com/wsq194/wsq194-workflow-manager
重启 ComfyUI 即可。

方法二：手动安装
下载本仓库 zip 并解压
将文件夹放入 ComfyUI/custom_nodes/
重启 ComfyUI
方法三：ComfyUI Manager
在 ComfyUI Manager 中搜索 wsq194 Workflow Manager 一键安装。
...
备份
右键工作流 → 💾 备份，将工作流 JSON 备份到本地 wsq194-workflows/backups/ 目录。

📂 数据存储
所有数据保存在 ComfyUI/wsq194-workflows/ 目录下：
...
验证方法
底部状态栏或者终端会显示 自动保存: HH:MM，等 60 秒看看有没有出现。

🙏 致谢
UI 设计和功能参考了 comfyui-workspace-manager，感谢原作者的开源贡献。
